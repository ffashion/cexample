...
int main(int argc, char **argv) {
	...
	char *daddr = "10.10.22.60";

	addr.sin_family = AF_INET;
	addr.sin_addr.s_addr = inet_addr(daddr);
	fd = make_raw_socket(IPPROTO_ICMP);
	//  need build a new icmp packet
	setup_icmphdr(ICMP_ECHO, 0, 0, 0, &icmphdr);
	//send icmp packet
	n = sendto(fd, (char *)&icmphdr, sizeof(icmphdr), 0, (struct sockaddr *)&addr, sizeof(addr));
	if (n < 1) {
		perror("sendto");
		return 1;
	}

	//have received icmp data, buf have ip header and icmp packet, now
	...
	/* 检查送信包的源地址匹配受信包的目的地址  */
	if (!strcmp(daddr, inet_ntoa(insaddr)) && recv_icmphdr->type == ICMP_ECHOREPLY)
